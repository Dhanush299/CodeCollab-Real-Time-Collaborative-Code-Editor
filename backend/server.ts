...
const filePresence = new Map<string, Map<string, Map<string, { username: string; role: string }>>>();

const roleRank: Record<string, number> = { viewer: 1, editor: 2, admin: 3 };
const normalizeRole = (role: any) => (role === 'admin' || role === 'editor' || role === 'viewer' ? role : 'viewer');
const hasAtLeastRole = (role: any, required: any) => (roleRank[normalizeRole(role)] || 1) >= (roleRank[normalizeRole(required)] || 1);

async function computeRoleForRoom(roomId: string, userId: string) {
  try {
    ...
    if (hostId && hostId === uid) return 'admin';

    if (Array.isArray(room.repository?.collaborators)) {
      const collab = room.repository.collaborators.find((c: any) => (c.user?.toString?.() || String(c.user)) === uid);
      if (collab?.role) return normalizeRole(collab.role);
    }

    const participant = room.participants?.find((p: any) => (p.user?.toString?.() || String(p.user)) === uid);
    if (participant?.role) return normalizeRole(participant.role);
...
io.on('connection', (socket) => {
  ...
  socket.on('leave-room', (data: any) => {
    ...
    if (filePresence.has(roomId)) {
      for (const [fid, usersMap] of filePresence.get(roomId)!.entries()) {
        ...
        if (usersMap.size === 0) filePresence.get(roomId)!.delete(fid);
      }
      if (filePresence.get(roomId)!.size === 0) filePresence.delete(roomId);
    }
  ...
  socket.on('file-focus', async (data: any) => {
    const { roomId, fileId } = data || {};
    const userId = (socket.data as any).userId || data?.userId;
    const username = (socket.data as any).username || data?.username;
    if (!roomId || !fileId || !userId) return;

    const role = await getRole(roomId, userId);
    if (!filePresence.has(roomId)) filePresence.set(roomId, new Map());
    if (!filePresence.get(roomId)!.has(fileId)) filePresence.get(roomId)!.set(fileId, new Map());
  ...
  socket.on('file-blur', (data: any) => {
    const { roomId, fileId } = data || {};
    const userId = (socket.data as any).userId || data?.userId;
    if (!roomId || !fileId || !userId) return;
    if (!filePresence.has(roomId)) return;
    if (!filePresence.get(roomId)!.has(fileId)) return;
    const usersMap = filePresence.get(roomId)!.get(fileId)!;
    usersMap.delete(String(userId));
    if (usersMap.size === 0) {
  ...
  // Code changes
  socket.on('code-change', async (data: any) => {
    const { roomId, fileId, content } = data;
    const userId = (socket.data as any).userId || data.userId;
    const role = await getRole(roomId, userId);
    if (!hasAtLeastRole(role, 'editor')) return;
  ...
  // Chat messages
  socket.on('send-message', async (data: any, cb?: (resp: any) => void) => {
  ...
  socket.on('drawing-update', async (data: any) => {
    ...
    if (drawingData.type === 'clear' || drawingData.type === 'undo-one' || drawingData.type === 'redo-one') {
      const username = (socket.data as any).username || data.username || String(userId);
  ...
  socket.on('update-participant-role', async (data: any, cb?: (resp: any) => void) => {
    const { roomId, targetUserId, role } = data || {};
    const actorUserId = (socket.data as any).userId;
    if (!roomId || !targetUserId || !actorUserId) return;
    ...
    try {
      const room: any = await Room.findOne({ roomId }).populate('repository', '_id collaborators owner').populate('host', '_id');
      if (!room) {
      ...
      const participant = room.participants.find((p: any) => (p.user?.toString?.() || String(p.user)) === tid);
      if (participant) {
      ...
      if (room.repository?._id) {
        const repo: any = await Repository.findById(room.repository._id);
        if (repo && Array.isArray(repo.collaborators)) {
          const collab = repo.collaborators.find((c: any) => (c.user?.toString?.() || String(c.user)) === tid);
          if (collab) {
            collab.role = newRole;
            await repo.save();
          }
      ...
      await logRoomActivity({
        roomId,
        actorId: actorUserId,
        actorUsername: (socket.data as any).username || '',
        type: 'role_change',
        message: `${(socket.data as any).username || 'Admin'} changed role of ${tid} to ${newRole}`,
        meta: { targetUserId: tid, role: newRole }
      });
      io.to(roomId).emit('room-activity', {
        type: 'role_change',
        message: `${(socket.data as any).username || 'Admin'} changed a user role to ${newRole}`,
        actorUsername: (socket.data as any).username || 'Admin',
        createdAt: new Date().toISOString(),
        meta: { targetUserId: tid, role: newRole }
      });
      cb?.({ ok: true });
  ...
  socket.on('kick-user', async (data: any, cb?: (resp: any) => void) => {
    ...
    try {
      ...
      const tid = String(targetUserId);
      room.participants = room.participants.filter((p: any) => (p.user?.toString?.() || String(p.user)) !== tid);
      await room.save();

      if (rooms.has(roomId)) rooms.get(roomId)!.delete(tid);
      if (roomRoles.has(roomId)) roomRoles.get(roomId)!.delete(tid);
      ...
      await logRoomActivity({
        ...
        actorUsername: (socket.data as any).username || '',
        type: 'kick',
        message: `${(socket.data as any).username || 'Admin'} removed a user from the room`,
