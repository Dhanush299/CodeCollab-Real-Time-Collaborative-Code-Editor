...
async function getAIResponse(systemPrompt, userPrompt, maxTokens = 500) {
...
router.post('/suggest', auth_1.auth, [
    (0, express_validator_1.body)('prompt').isString().isLength({ min: 1, max: 1000 }),
    (0, express_validator_1.body)('language').isString(),
    (0, express_validator_1.body)('context').optional().isString().isLength({ max: 5000 }),
    (0, express_validator_1.body)('cursorPosition').optional().isNumeric()
], async (req, res) => {
    try {
        ...
        const { prompt, language, context = '', cursorPosition } = req.body;
...
router.post('/explain', auth_1.auth, [(0, express_validator_1.body)('code').isString().isLength({ min: 1, max: 5000 }), (0, express_validator_1.body)('language').isString()], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ errors: errors.array() });
...
router.post('/generate', auth_1.auth, [(0, express_validator_1.body)('description').isString().isLength({ min: 1, max: 1000 }), (0, express_validator_1.body)('language').isString(), (0, express_validator_1.body)('context').optional().isString().isLength({ max: 2000 })], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ errors: errors.array() });
...
// Debug/improve code
router.post('/debug', auth_1.auth, [(0, express_validator_1.body)('code').isString().isLength({ min: 1, max: 5000 }), (0, express_validator_1.body)('language').isString(), (0, express_validator_1.body)('error').optional().isString().isLength({ max: 1000 })], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ errors: errors.array() });
        }
        const { code, language, error: errorMsg = '' } = req.body;
        if (!openai && !gemini) {
            return res.status(503).json({ message: 'AI service not available. Please configure OPENAI_API_KEY or GEMINI_API_KEY' });
        }
        try {
